...
function [xmma,ymma,zmma,lam,xsi,eta,mu,zet,s,low,upp] = mmasub(m,n,iter,xval,xmin,xmax,xold1,xold2, f0val,df0dx,df0dx2,fval,dfdx,dfdx2,low,upp,a0,a,c,d);

epsimin = sqrt(m+n)*10^(-9);
feps = 0.000001;

asyinit = 0.5;                          % FOR INITIAL ASYMS : SMALL asyinit -> MORE CONVEX (0.5)
asyincr = 1.1;                         % INCREASE ASYMS : LARGE asyincr -> LESS CONVEX (1.2)
asydecr = 0.6;                        % DECREASE ASYMS : LARGE asydecr -> LESS CONVEX (0.7) 

albefa = 0.95;                           % MOVE LIMIT : LARGE albefa -> SMALL MOVE (0.1<1)
een = ones(n,1);
zeron = zeros(n,1);
